...
abstract class AbstractChartTest extends ImagineTestCase
{
    public function getLineChart()
    {
        ...
        $data =[];
        for($x=-6; $x <= 6; $x+=0.5) {$data[] = [$x, $x*$x];}
        $chart->addDataSet(
            new DataSet(
                $data,
                new LineStyle(
                    new RGB(new RGBPalette(), [0, 128, 0], 100),
        ...
        $chart->addDataSet(
            new DataSet(
                $data,
                new LineStyle(
                    new RGB(new RGBPalette(), [128, 0, 0], 100),
        ...
        $data =[];
        for($x=-6; $x <= 6; $x+=0.25) {$data[] = [$x, 5 * sin($x)];}
        $chart->addDataSet(
            new DataSet(
                $data,
                new LineStyle(
                    new RGB(new RGBPalette(), [0, 0, 128], 100),
